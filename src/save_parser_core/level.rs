...
fn parse_headers_and_level(
    ...
    let level_name = if persistent_level_flag {
        None
    } else {
        Some(c.string()?)
    };
    ...
    for _ in 0..actor_and_component_count {
        ...
        if let Some(cb) = progress.as_deref_mut() {
            if c.pos - last_report > 1 << 20 {
                cb(
                    1,
                    progress_base + (c.pos - level_start) as u64,
                    progress_total,
                );
                last_report = c.pos;
            }
    ...
    let level_save_version = if header_save_version >= FIRST_1_0_SAVE_VERSION {
        c.u32()?
    } else {
        header_save_version
    };
        ...
        if let Some(cb) = progress.as_deref_mut() {
            if oc.pos - last_report > 1 << 20 {
                cb(
                    1,
                    progress_base
                        + (oc.pos - object_start + header_start - level_start) as u64
...
fn parse_full_save_impl(
    ...
    let decompressed = {
        let mut cb_adapter = progress
            .as_deref_mut()
            .map(|cb| move |cur: u64, total: u64| cb(0, cur, total));
