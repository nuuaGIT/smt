...
pub fn parse_text_property(c: &mut Cursor) -> PResult<TextValue> {
    ...
    match history_type {
        255 => {
            let invariant = c.u32()?;
            let s = if invariant != 0 {
                c.string()?
            } else {
                crate::reader::EMPTY_STR
            };
            Ok(TextValue::NoneHistory {
                flags,
                invariant,
                s,
            })
    ...
    loop {
        ...
        let value: PropertyValue = match ptype {
            ...
            "ByteProperty" => {
                ...
                let start = c.pos;
                let en = if enum_name_is_none_value {
                    None
                } else {
                    enum_name
                };
            ...
            "ArrayProperty" => {
                ...
                let av: ArrayValue = match at_s.as_ref() {
                    ...
                    "StructProperty" => {
                        if !property_header_flag {
                            let name = c.string()?;
                            if name.bytes(c.data) != property_name.bytes(c.data)
                                || name.wide != property_name.wide
                            {
                                return Err(perr!(
                        ...
                        let sst = structure_sub_type.ok_or_else(|| {
                            perr!("ArrayProperty StructProperty without structureSubType")
                        })?;
                        let sst_s = type_str(sst, c.data);
                        let inner: ArrayValue = match sst_s.as_ref() {
                            "LinearColor" => {
                                let mut v =
                                    Vec::with_capacity(c.capped_capacity(array_count as usize, 16));
                                for _ in 0..array_count {
                            ...
                            "Vector" => {
                                let mut v =
                                    Vec::with_capacity(c.capped_capacity(array_count as usize, 12));
                                for _ in 0..array_count {
                            ...
                            | "USSSwatchSaveInfo" => {
                                let mut v =
                                    Vec::with_capacity(c.capped_capacity(array_count as usize, 4));
                                for _ in 0..array_count {
                                    v.push(parse_properties(
                                        c,
                            ...
                            other => {
                                ...
                                let remaining = (property_size as usize)
                                    .checked_sub(c.pos - start)
                                    .ok_or_else(|| {
                                        perr!(
            ...
            "StructProperty" => {
                ...
                let sv: StructValue = match spt_s.as_ref() {
                    ...
                    "LinearColor" => {
                        StructValue::LinearColor([c.f32()?, c.f32()?, c.f32()?, c.f32()?])
                    }
                    ...
                    "ClientIdentityInfo" => {
                        ...
                        let mut identities =
                            Vec::with_capacity(c.capped_capacity(identity_count as usize, 4));
            ...
            "MapProperty" => {
                ...
                for _ in 0..n {
                    ...
                    let v = match vt_s.as_ref() {
                        ...
                        "StrProperty" | "NameProperty" | "EnumProperty" => MapVal::Str(c.string()?),
